...
initiative, no minting, public pre-sale, or distribution of the INDY tokens have been undertaken. This ensures that starting from
...
\begin{itemize}
\item
  Tracking different types of assets and statistics; which allows the
...
Generally, an iAsset name's begins with the letter ``i'', followed by the
...
buying an iAsset, the user gains full control of the iAsset and can reap any benefits available. Users can see that
...
Cardano, and rewards earned through Indigo. Liquid staking is a unique
...
Launch}, therefore there has been no minting, public pre-sale nor private distribution of INDY to investors prior to launch.
...
codebase -- and who are available to continue to improve, optimize, and develop new
...
INDY tokens to third parties. The purpose of INDY is to be used within the
...
\begin{enumerate}
\item
  Volatility of the SP's iAsset's underlying asset (\(\sigma\))
\item
...
\hypertarget{volatility-for-a-sp-sigma}{%
\paragraph{\texorpdfstring{Volatility for a SP
(\(\sigma\))}{Volatility for a SP (\textbackslash sigma)}}\label{volatility-for-a-sp-sigma}}

can be calculated using the formula:

\[\sigma = \left\{ \begin{matrix}
\begin{pmatrix}
\text{let}\text{\ y\ }\text{equal}\ \left\langle \frac{x_{j}}{x_{j - 1}} - 1\ :j\  \in \ \left\langle 2,\ldots,\left| x \right| \right\rangle \right\rangle \\
\  \\
\sqrt{\frac{\sum_{i = 1}^{\left| y \right|}\left( y_{i} - \overline{y} \right)^{2}}{\left| y \right|}} \\
\end{pmatrix} & \text{if}\ \left| x \right| > 30 \\
\  & \  \\
...
\begin{itemize}
\item
  \(\sigma\) is the volatility of the iAsset
\item
  \(x\) is the sequence of the iAsset's tracked asset's historical daily
  close price for the past year (or maximum amount of time that data
  exists for) denominated in iAsset's collateral (i.e., ADA)
\end{itemize}
...
\[\rho = \begin{pmatrix}
\text{let}\ \sigma_{s}\ \text{equal}\ \sum_{i = 1}^{\left| \sigma \right|}\left\{ \begin{matrix}
\frac{1}{\sigma_{i}} & \text{if}\ \sigma_{i} > 0\ \text{and}\ t_{i} > 0 \\
\  & \  \\
0 & \text{otherwise} \\
\end{matrix} \right.\  \\
\  \\
...
\  & \  \\
\begin{pmatrix}
\text{let}\ \sigma_{v}\ \text{equal}\ \left\{ \begin{matrix}
\frac{1}{\sigma_{a}}/\sigma_{s} & \text{if}\ \sigma_{a} > 0\ \text{and}\ \sigma_{s} > 0 \\
\  & \  \\
0 & \text{otherwise} \\
\end{matrix} \right.\  \\
\  \\
...
\  \\
\left\{ \begin{matrix}
\frac{\sigma_{v} + \varphi_{v} + \omega_{v}}{3} & \text{if}\ \varphi_{s} > 0\ \text{and}\ \omega_{s} > 0 \\
\  & \  \\
\sigma_{v} & \text{otherwise} \\
\end{matrix} \right.\  \\
...
\item
  \(\sigma\) is the sequence of volatility values of each iAsset
\item
...
Unclaimed rewards are withdrawable for three months. Any rewards not
claimed within three months after being rewarded are redistributed to
Members via the \protect\hyperlink{protocol-fee-distribution}{Collector}.
...
\item
  ...
  by the Indigo Foundation to launch the protocol and available for the continued development of Indigo and
...
have possession of or control of any user funds. The Foundation only receives INDY when provided by the DAO to fund the Foundation and to make payments to third party vendors.
...
\textbf{Step 1 -- Temperature Check}: A user who does not have to be an INDY token holder or Member - creates and submits their
...
positive outcome, a user needs to deposit INDY to submit the proposal for 
on-chain voting. In addition, the user submitting the proposal should also
...
set to the total amount of INDY staked at the time of casting. If a
...
already voted on. If the Member deposits additional INDY after casting a vote and
...
Laboratories will then be hired to begin work on building software to implement the
new features.

When software is ready for deployment, a proposal to upgrade is initiated, possibly including a request for independent audit. Members can inspect the new code which has been requested to be deployed and either approve or reject the proposal. Upon approval (and potentially the approval of any audit), the developing
entity of the software can deploy the code to Cardano, and the protocol will
...
After deployment, individual user positions can be migrated
from the old version to the new one. Some protocol features may not be available
...
migrate their CDP to the new version of the protocol. To migrate a CDP, a user
...
\hypertarget{protocol-fee-distribution}{%
\subsection{Protocol Fee Distribution}\label{protocol-fee-distribution}}
...
fees. The fee is set at launch to 2\% and modifiable by vote of Members.
...
INDY stakers. Users who stake their INDY (Members) are eligible to a share of all
