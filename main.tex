...
\[\rho = \begin{pmatrix}
\text{let}\ \sigma_{s}\ \text{equal}\ \sum_{i = 0}^{\left| \sigma \right|}\left\{ \begin{matrix}
\frac{1}{\sigma_{i}} & \text{if}\ \sigma_{i} > 0\ \text{and}\ t_{i} > 0 \\
\  & \  \\
0 & \text{otherwise} \\
\end{matrix} \right.\  \\
\  \\
\text{let}\ \varphi_{s}\ \text{equal}\ \sum_{i = 0}^{\left| \varphi \right|}\left\{ \begin{matrix}
\frac{1}{\varphi_{i}} & \text{if}\ \varphi_{i} > 0\ \text{and}\ t_{i} > 0 \\
\  & \  \\
0 & \text{otherwise} \\
\end{matrix} \right.\  \\
\  \\
\text{let}\ \omega_{s}\ \text{equal}\ \sum_{i = 0}^{\left| \omega \right|}\left\{ \begin{matrix}
...
\item
  ...
  each iAsset
\item
  \(\omega\) is the sequence of market caps of each iAsset
\item
  \(t\) is the sequence of total number of Stability Pool stakers, who
  have staked for at least 24 hours, of each iAsset
\item
  \(a\) is the index of the iAsset to caculcate \(\rho\) for
  corresponding to the \(\sigma\), \(\varphi\), \(\omega\), and \(t\)
  sequences
\end{itemize}
