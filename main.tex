...
from the stake pool, accuring in the user's wallet.

After a CDP is created, its stake key cannot be changed. If the user's
wallet delegates to a new stake pool after creating the CDP, the CDP
will continue to earn rewards using the original stake pool. To delegate
to a different stake pool, users will need to close their CDP and open a
new CDP, which will then attach the new CDP with the wallet's newly
